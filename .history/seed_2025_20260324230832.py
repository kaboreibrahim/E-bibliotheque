...
def seed_ues_ecues():
    ...
    # ── Récupérer la structure créée ─────────────────────────────────────────────
    filiere_tc = Filiere.objects.get(name="Droit (Tronc Commun)")
    
    specialites = ["Droit des Contentieux", "Droit des Affaires", "Droit Public", "Droit Privé"]
    filieres_master = {}
    niveaux_master = {}
    
    for spec in specialites:
        filieres_master[spec] = Filiere.objects.get(name=spec)
        niveaux_master[f"{spec}_M1"] = Niveau.objects.get(filiere=filieres_master[spec], name='M1')
        niveaux_master[f"{spec}_M2"] = Niveau.objects.get(filiere=filieres_master[spec], name='M2')

    # ── UEs Tronc Commun (L1/L2/L3) ───────────────────────────────────────────────
    ues_tronc_commun = [
        {'code': 'DGL101', 'name': 'Droit Constitutionnel Général', 'coef': 3.00},
        {'code': 'DGL102', 'name': 'Droit Civil des Personnes', 'coef': 3.00},
        {'code': 'DGL103', 'name': 'Droit Penal Général', 'coef': 2.50},
        {'code': 'DGL104', 'name': 'Introduction au Droit Administratif', 'coef': 2.50},
        {'code': 'DGL105', 'name': 'Histoire du Droit', 'coef': 2.00},
        {'code': 'DGL106', 'name': 'Droit des Obligations', 'coef': 3.00},
        {'code': 'DGL107', 'name': 'Droit Constitutionnel Ivoirien', 'coef': 2.50},
        {'code': 'DGL108', 'name': 'Droit Administratif Général', 'coef': 2.50},
    ]

    for ue_data in ues_tronc_commun:
        ue, created = UE.objects.get_or_create(
            code=ue_data['code'],
            defaults={'name': ue_data['name'], 'coef': ue_data['coef']}
        )
        # Associer à tous les niveaux L1, L2, L3 du tronc commun
        for niveau_name in ['L1', 'L2', 'L3']:
            niveau = Niveau.objects.get(filiere=filiere_tc, name=niveau_name)
            ue.niveaux.add(niveau)
        if created:
            print(f"   ✔ UE Tronc Commun : {ue.code} — {ue.name}")

    # ── UEs Droit des Contentieux (M1/M2) ───────────────────────────────────────────
    ues_contentieux_m1 = [
    ...
    for ue_data in ues_contentieux_m1:
        ue, created = UE.objects.get_or_create(
            code=ue_data['code'],
            defaults={'name': ue_data['name'], 'coef': ue_data['coef']}
        )
        ue.niveaux.add(niveaux_master["Droit des Contentieux_M1"])
        if created:
            print(f"   ✔ UE Contentieux M1 : {ue.code} — {ue.name}")

    for ue_data in ues_contentieux_m2:
        ue, created = UE.objects.get_or_create(
            code=ue_data['code'],
            defaults={'name': ue_data['name'], 'coef': ue_data['coef']}
        )
        ue.niveaux.add(niveaux_master["Droit des Contentieux_M2"])
        if created:
            print(f"   ✔ UE Contentieux M2 : {ue.code} — {ue.name}")

    # ── UEs Droit des Affaires (M1/M2) ─────────────────────────────────────────────
    ues_affaires_m1 = [
        {'code': 'DAC3101', 'name': 'Droit des Sociétés', 'coef': 3.00},
        {'code': 'DAC3102', 'name': 'Droit Commercial Général', 'coef': 2.50},
        {'code': 'DAC3103', 'name': 'Droit Bancaire et Financier', 'coef': 2.50},
        {'code': 'DAC3104', 'name': 'Droit Fiscal des Entreprises', 'coef': 2.00},
        {'code': 'DAC3105', 'name': 'Droit de la Concurrence', 'coef': 2.00},
    ]

    ues_affaires_m2 = [
        {'code': 'DAC3201', 'name': 'Droit International des Affaires', 'coef': 3.00},
        {'code': 'DAC3202', 'name': 'Droit Boursier et Marchés Financiers', 'coef': 2.50},
        {'code': 'DAC3203', 'name': 'Propriété Intellectuelle', 'coef': 2.50},
        {'code': 'DAC3204', 'name': 'Droit des Nouvelles Technologies', 'coef': 2.00},
        {'code': 'TDA3205', 'name': 'Mémoire Droit des Affaires', 'coef': 4.00},
    ]

    for ue_data in ues_affaires_m1:
        ue, created = UE.objects.get_or_create(
            code=ue_data['code'],
            defaults={'name': ue_data['name'], 'coef': ue_data['coef']}
        )
        ue.niveaux.add(niveaux_master["Droit des Affaires_M1"])
        if created:
            print(f"   ✔ UE Affaires M1 : {ue.code} — {ue.name}")

    for ue_data in ues_affaires_m2:
        ue, created = UE.objects.get_or_create(
            code=ue_data['code'],
            defaults={'name': ue_data['name'], 'coef': ue_data['coef']}
        )
        ue.niveaux.add(niveaux_master["Droit des Affaires_M2"])
        if created:
            print(f"   ✔ UE Affaires M2 : {ue.code} — {ue.name}")

    # ── UEs Droit Public (M1/M2) ───────────────────────────────────────────────────
    ues_public_m1 = [
        {'code': 'DPB4101', 'name': 'Droit Administratif Approfondi', 'coef': 3.00},
        {'code': 'DPB4102', 'name': 'Droit Constitutionnel Comparé', 'coef': 2.50},
        {'code': 'DPB4103', 'name': 'Droit des Libertés Fondamentales', 'coef': 2.50},
        {'code': 'DPB4104', 'name': 'Droit Public Économique', 'coef': 2.00},
        {'code': 'DPB4105', 'name': 'Droit de l\'Urbanisme', 'coef': 2.00},
    ]

    ues_public_m2 = [
        {'code': 'DPB4201', 'name': 'Droit International Public', 'coef': 3.00},
        {'code': 'DPB4202', 'name': 'Droit Communautaire', 'coef': 2.50},
        {'code': 'DPB4203', 'name': 'Droit de l\'Environnement', 'coef': 2.50},
        {'code': 'DPB4204', 'name': 'Droit Fiscal Public', 'coef': 2.00},
        {'code': 'TDP4205', 'name': 'Mémoire Droit Public', 'coef': 4.00},
    ]

    for ue_data in ues_public_m1:
        ue, created = UE.objects.get_or_create(
            code=ue_data['code'],
            defaults={'name': ue_data['name'], 'coef': ue_data['coef']}
        )
        ue.niveaux.add(niveaux_master["Droit Public_M1"])
        if created:
            print(f"   ✔ UE Public M1 : {ue.code} — {ue.name}")

    for ue_data in ues_public_m2:
        ue, created = UE.objects.get_or_create(
            code=ue_data['code'],
            defaults={'name': ue_data['name'], 'coef': ue_data['coef']}
        )
        ue.niveaux.add(niveaux_master["Droit Public_M2"])
        if created:
            print(f"   ✔ UE Public M2 : {ue.code} — {ue.name}")

    # ── UEs Droit Privé (M1/M2) ───────────────────────────────────────────────────
    ues_prive_m1 = [
        {'code': 'DPR5101', 'name': 'Droit Civil Approfondi', 'coef': 3.00},
        {'code': 'DPR5102', 'name': 'Droit des Contrats Spéciaux', 'coef': 2.50},
        {'code': 'DPR5103', 'name': 'Droit des Sûretés', 'coef': 2.50},
        {'code': 'DPR5104', 'name': 'Droit de la Famille', 'coef': 2.00},
        {'code': 'DPR5105', 'name': 'Droit des Successions', 'coef': 2.00},
    ]

    ues_prive_m2 = [
        {'code': 'DPR5201', 'name': 'Droit International Privé', 'coef': 3.00},
        {'code': 'DPR5202', 'name': 'Droit des Responsabilités', 'coef': 2.50},
        {'code': 'DPR5203', 'name': 'Droit Immobilier', 'coef': 2.50},
        {'code': 'DPR5204', 'name': 'Droit Pénal des Affaires', 'coef': 2.00},
        {'code': 'TDR5205', 'name': 'Mémoire Droit Privé', 'coef': 4.00},
    ]

    for ue_data in ues_prive_m1:
        ue, created = UE.objects.get_or_create(
            code=ue_data['code'],
            defaults={'name': ue_data['name'], 'coef': ue_data['coef']}
        )
        ue.niveaux.add(niveaux_master["Droit Privé_M1"])
        if created:
            print(f"   ✔ UE Privé M1 : {ue.code} — {ue.name}")

    for ue_data in ues_prive_m2:
        ue, created = UE.objects.get_or_create(
            code=ue_data['code'],
            defaults={'name': ue_data['name'], 'coef': ue_data['coef']}
        )
        ue.niveaux.add(niveaux_master["Droit Privé_M2"])
        if created:
            print(f"   ✔ UE Privé M2 : {ue.code} — {ue.name}")

    print("   ✔ Toutes les UEs créées avec leurs spécialités")
