...
from apps.users.models import User, Etudiant, Bibliothecaire, PersonneExterne, EnseignantChercheur, Chercheur
...
class EnseignantChercheurInline(admin.StackedInline):
    """Profil enseignant-chercheur affiché dans la fiche utilisateur."""
    model = EnseignantChercheur
    can_delete = False
    verbose_name = "Profil Enseignant-Chercheur"
    fields = ('numero_piece', 'lieu_habitation', 'annee_service', 'specialite_enseignee', 'activation_suspendue')
    extra = 0


class ChercheurInline(admin.StackedInline):
    """Profil chercheur affiché dans la fiche utilisateur."""
    model = Chercheur
    can_delete = False
...
class UserAdmin(BaseUserAdmin, ExportCsvMixin):
    ...
    # ── Inline profils ────────────────────────────────────────────────────────
    inlines = [EtudiantInline, BibliothacaireInline, PersonneExterneInline, EnseignantChercheurInline, ChercheurInline]
    ...
    def user_type_badge(self, obj):
        colors = {
            ...
            'PERSONNE_EXTERNE':     '#6c757d',
            'ENSEIGNANT_CHERCHEUR': '#9c27b0',
            'CHERCHEUR':            '#e91e63',
        }
    ...
    def activer_comptes(self, request, queryset):
        ...
        for user in queryset.select_related(
            ...
            if hasattr(user, 'profil_enseignant_chercheur'):
                user.profil_enseignant_chercheur.reactiver_compte()
                nb += 1
                continue

            if hasattr(user, 'profil_chercheur'):
                user.profil_chercheur.reactiver_compte()
                nb += 1
                continue
    ...
    def desactiver_comptes(self, request, queryset):
        ...
        for user in queryset.exclude(id=request.user.id).select_related(
            ...
            if hasattr(user, 'profil_enseignant_chercheur'):
                user.profil_enseignant_chercheur.suspendre_compte()
                nb += 1
                continue

            if hasattr(user, 'profil_chercheur'):
                user.profil_chercheur.suspendre_compte()
                nb += 1
                continue
...
@admin.register(EnseignantChercheur)
class EnseignantChercheurAdmin(ExportCsvMixin, admin.ModelAdmin):

    list_display = (
        'user_email', 'user_nom', 'specialite_enseignee',
        'annee_service', 'statut_badge', 'created_at',
    )
    list_filter = ('activation_suspendue',)
    search_fields = ('user__email', 'user__first_name', 'user__last_name',
                     'specialite_enseignee', 'numero_piece')
    autocomplete_fields = ('user',)
    list_select_related = ('user',)
    ordering = ('user__last_name',)
    list_per_page = 25
    ...
    fieldsets = (
        ...
        ("Informations professionnelles", {
            'fields': ('numero_piece', 'lieu_habitation', 'annee_service', 'specialite_enseignee')
        }),
        ("Statut du compte", {'fields': ('activation_suspendue',)}),
        ("Dates", {'fields': ('created_at', 'updated_at'), 'classes': ('collapse',)}),
    )

    actions = ['suspendre_comptes', 'reactiver_comptes', 'export_csv']
    ...
    def statut_badge(self, obj):
        if obj.activation_suspendue:
            return mark_safe('<span style="color:#dc3545;font-weight:bold;">⛔ Suspendu</span>')
        return mark_safe('<span style="color:#28a745;font-weight:bold;">✅ Actif</span>')

    @admin.action(description="⛔ Suspendre les comptes sélectionnés")
    def suspendre_comptes(self, request, queryset):
        nb = sum(1 for obj in queryset if not obj.activation_suspendue and (obj.suspendre_compte() or True))
        self.message_user(request, f"{nb} compte(s) suspendu(s).")

    @admin.action(description="✅ Réactiver les comptes sélectionnés")
    def reactiver_comptes(self, request, queryset):
        nb = sum(1 for obj in queryset if obj.activation_suspendue and (obj.reactiver_compte() or True))
        self.message_user(request, f"{nb} compte(s) réactivé(s).")
...
class ChercheurAdmin(ExportCsvMixin, admin.ModelAdmin):
...
class NiveauAdmin(admin.ModelAdmin):
    ...
    list_display   = ('filiere', 'name', 'types_documents_badge', 'nb_specialites', 'nb_etudiants', 'nb_documents')
    ...
    def get_queryset(self, request):
        return (
            ...
            .prefetch_related('specialites', 'types_documents')
            .annotate(
    ...
    @admin.display(description="Types de documents")
    def types_documents_badge(self, obj):
        types = obj.types_documents.all()
        if not types:
            return mark_safe('<span style="color:#94a3b8; font-size:12px;">—</span>')
        _colors = {
            "COURS":   ("#DCFCE7", "#16A34A", "#BBF7D0"),
            "EXAMEN":  ("#FEF3C7", "#D97706", "#FDE68A"),
            "MEMOIRE": ("#EDE9FE", "#7C3AED", "#DDD6FE"),
            "THESE":   ("#FCE7F3", "#DB2777", "#FBCFE8"),
        }
        badges = []
        for t in types:
            bg, fg, border = _colors.get(t.code, ("#F1F5F9", "#475569", "#E2E8F0"))
            badges.append(format_html(
                '<span style="display:inline-block;padding:2px 8px;margin:1px;border-radius:12px;'
                'font-size:11px;font-weight:600;background:{};color:{};border:1px solid {};">{}</span>',
                bg, fg, border, t.name,
            ))
...
class TypeDocumentAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'code', 'ordre', 'niveaux_badge', 'nb_documents', 'created_at')
    list_editable = ['ordre']
    list_filter = ('niveaux__name', 'niveaux__filiere')
    search_fields = ('code', 'name')
    ordering = ('ordre', 'name')
    list_per_page = 25
    ...
    fieldsets = [
        ...
        ("Affichage & classement", {
            "fields": ["ordre"],
            "description": "Définit l'ordre d'apparition dans les listes (0 = en premier).",
        }),
        ("Niveaux associés", {
            "fields": ["niveaux"],
            "description": "Associez ce type de document aux niveaux d'études concernés.",
        }),
    ]
    ...
    def get_queryset(self, request):
        return (
            super().get_queryset(request)
            .prefetch_related('niveaux', 'niveaux__filiere')
            .annotate(_nb_documents=Count('documents', distinct=True))
        )

    @admin.display(description="Niveaux")
    def niveaux_badge(self, obj):
        niveaux = obj.niveaux.all()
        if not niveaux:
            return mark_safe('<span style="color:#94a3b8;font-size:12px;">—</span>')
        badges = []
        for n in niveaux:
            badges.append(format_html(
                '<span style="display:inline-block;padding:2px 8px;margin:1px;border-radius:12px;'
                'font-size:11px;font-weight:600;background:#EFF6FF;color:#1D4ED8;border:1px solid #BFDBFE;">{}</span>',
                n.name,
            ))
        return mark_safe(' '.join(str(b) for b in badges))
...
@admin.register(DocumentStorageConfiguration)
class DocumentStorageConfigurationAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        if DocumentStorageConfiguration.objects.exists():
            return False
    ...
    @admin.display(description='Capacite formatee')
    def capacity_display(self, obj):
        return obj.server_disk_capacity_display
...
class DocumentAdmin(ExportCsvMixin, admin.ModelAdmin):
    ...
    search_fields  = ('title', 'auteur', 'description', 'specialite__name', 'ue__code', 'ue__name')
    ...
    fieldsets = (
        (_("📋 Informations générales"), {
            'fields': ('title', 'type', 'annee_academique_debut', 'description', 'file_path', 'file_name', 'file_mime_type')
        }),
        (_("🏫 Classification"), {
