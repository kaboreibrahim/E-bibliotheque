...
class EnseignantChercheurAuthService:
    """Authentification enseignant-chercheur : email + mot de passe + TOTP."""

    @staticmethod
    def login(email: str, password: str, ip: str = None, ua: str = None) -> AuthResult:
        user = UserRepository.get_by_email(email)

        if not user or user.user_type != 'ENSEIGNANT_CHERCHEUR':
            HAS.log_connexion(user=None, statut='echec', ip=ip, ua=ua)
            return AuthResult(
                success=False,
                message="Email ou mot de passe incorrect.",
        ...
        if not user.totp_secret or not user.is_2fa_enabled:
            return CommonAuthService.begin_first_login_totp_setup(user=user, ip=ip, ua=ua)
    ...
    def verify_totp(user_id: str, totp_code: str, ip: str = None) -> AuthResult:
        user = UserRepository.get_by_id(user_id)
        if not user or user.user_type != 'ENSEIGNANT_CHERCHEUR':
            return AuthResult(success=False, message="Enseignant-chercheur introuvable.", http_status=404)
...
class ChercheurAuthService:
    """Authentification chercheur : email + mot de passe + TOTP."""

    @staticmethod
    def login(email: str, password: str, ip: str = None, ua: str = None) -> AuthResult:
        user = UserRepository.get_by_email(email)

        if not user or user.user_type != 'CHERCHEUR':
            HAS.log_connexion(user=None, statut='echec', ip=ip, ua=ua)
            return AuthResult(
                success=False,
                message="Email ou mot de passe incorrect.",
    ...
    def verify_totp(user_id: str, totp_code: str, ip: str = None) -> AuthResult:
        user = UserRepository.get_by_id(user_id)
        if not user or user.user_type != 'CHERCHEUR':
            return AuthResult(success=False, message="Chercheur introuvable.", http_status=404)
...
def _send_otp_email(user: User, code: str, type_code: str) -> None:
    """Envoie le code OTP par email avec template HTML stylisé."""
    subjects = {
        'activation':     'Activation de votre compte — Bibliothèque Universitaire',
        'password_reset': 'Réinitialisation de votre mot de passe — Bibliothèque Universitaire',
        'email_change':   "Confirmation de changement d'email — Bibliothèque Universitaire",
        'code_OTP':       'Votre code de connexion — Bibliothèque Universitaire',
    }
    templates = {
        'activation':     'emails/activation.html',
        'password_reset': 'emails/password_reset.html',
        'email_change':   'emails/email_change.html',
        'code_OTP':       'emails/otp_code.html',
    }
    expiry_minutes = 3
    try:
        ...
        template_name = templates.get(type_code, 'emails/otp_code.html')
        html_content  = render_to_string(template_name, context)

        text_content = (
            f"Bonjour {context['user_name']},\n\n"
            f"Votre code : {code}\n\n"
            f"Ce code est valable {expiry_minutes} minutes. Ne le partagez avec personne.\n\n"
            f"— Bibliothèque Universitaire"
        )

        msg = EmailMultiAlternatives(
            subject=subjects.get(type_code, 'Votre code OTP — Bibliothèque Universitaire'),
            body=text_content,
