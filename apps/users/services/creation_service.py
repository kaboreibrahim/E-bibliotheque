...
class EtudiantCreationService:
    ...
    def creer_etudiant(
        ...
        # La periode de validite (date_debut_validite/date_fin_validite) est
        # heritee automatiquement de l'annee academique courante par
        # Etudiant.save() si elle n'est pas fournie ici (voir etudiant_models.py).
...
# =============================================================================
# 👨‍🏫  SERVICE CREATION ENSEIGNANT-CHERCHEUR
# =============================================================================

class EnseignantChercheurCreationService:
    """
    Logique metier pour la creation d'un compte enseignant-chercheur.
    Reserve a l'administrateur uniquement.
    """

    @staticmethod
    def creer(data: dict, effectue_par: User) -> ServiceResult:
        if UserRepository.exists_by_email(data['email']):
            return ServiceResult(
                success=False,
                message="Un compte avec cet email existe deja.",
        ...
        if UserRepository.exists_by_phone(data['phone']):
            return ServiceResult(
                success=False,
                message="Un compte avec ce numero de telephone existe deja.",
                errors={'phone': ['Telephone deja utilise.']},
                http_status=400,
            )
        ...
        user = UserRepository.create(
            ...
            last_name=data['last_name'],
            phone=data['phone'],
            user_type='ENSEIGNANT_CHERCHEUR',
            date_of_birth=data.get('date_of_birth'),
            is_active=True,
        )
        ...
        profil = EnseignantChercheurRepository.create(
            user=user,
            numero_piece=data.get('numero_piece', ''),
            lieu_habitation=data.get('lieu_habitation', ''),
        ...
        HAS.log_utilisateur(
            ...
            cible_user=user,
            details={
                'profil_type': 'ENSEIGNANT_CHERCHEUR',
                'annee_service': profil.annee_service,
                'specialite_enseignee': profil.specialite_enseignee,
            },
        )

        return ServiceResult(
            success=True,
            message=f"Compte enseignant-chercheur créé avec succès pour {user.get_full_name()}.",
            data={
                'enseignant_chercheur_id': str(profil.id),
                'user_id': str(user.id),
                'email': user.email,
                'nom_complet': user.get_full_name(),
                'user_type': user.user_type,
                'statut_compte': profil.statut_compte,
                'annee_service': profil.annee_service,
                'specialite_enseignee': profil.specialite_enseignee,
                'totp_setup': {
                    'totp_secret': totp_secret,
                    'qr_uri': qr_uri,
                    'instructions': (
                        "Transmettez ces informations à l'enseignant-chercheur. "
                        "Il devra scanner ce QR code avec Google Authenticator "
                        "et confirmer via POST /api/auth/totp/confirm/."
                    ),
                },
    ...
    @staticmethod
    def suspendre(profil_id: str, effectue_par: User) -> ServiceResult:
        profil = EnseignantChercheurRepository.get_by_id(profil_id)
        if not profil:
            return ServiceResult(success=False, message="Enseignant-chercheur introuvable.", http_status=404)
        if profil.activation_suspendue:
            return ServiceResult(success=False, message="Ce compte est deja suspendu.", http_status=400)

        EnseignantChercheurRepository.suspendre(profil)
        HAS.log_utilisateur('DESACTIVATION', auteur=effectue_par, cible_user=profil.user, details={'action': 'suspension'})
        return ServiceResult(success=True, message="Compte suspendu.", data={'statut_compte': profil.statut_compte}, http_status=200)

    @staticmethod
    def reactiver(profil_id: str, effectue_par: User) -> ServiceResult:
        profil = EnseignantChercheurRepository.get_by_id(profil_id)
        if not profil:
            return ServiceResult(success=False, message="Enseignant-chercheur introuvable.", http_status=404)
        if not profil.activation_suspendue:
            return ServiceResult(success=False, message="Ce compte n'est pas suspendu.", http_status=400)

        EnseignantChercheurRepository.reactiver(profil)
        HAS.log_utilisateur('REACTIVATION', auteur=effectue_par, cible_user=profil.user, details={'action': 'reactivation'})
        return ServiceResult(success=True, message="Compte reactive.", data={'statut_compte': profil.statut_compte}, http_status=200)


# =============================================================================
# 🔬  SERVICE CREATION CHERCHEUR
# =============================================================================

class ChercheurCreationService:
    """
    Logique metier pour la creation d'un compte chercheur.
    Reserve a l'administrateur uniquement.
    """
    ...
    def creer(data: dict, effectue_par: User) -> ServiceResult:
        ...
        profil = ChercheurRepository.create(
            user=user,
            numero_piece=data.get('numero_piece', ''),
            lieu_habitation=data.get('lieu_habitation', ''),
            annee_service=data.get('annee_service'),
        ...
        return ServiceResult(
            success=True,
            message=f"Compte chercheur créé avec succès pour {user.get_full_name()}.",
            data={
                'chercheur_id': str(profil.id),
                'user_id': str(user.id),
                'email': user.email,
                'nom_complet': user.get_full_name(),
                'user_type': user.user_type,
                ...
                'totp_setup': {
                    ...
                    'qr_uri': qr_uri,
                    'instructions': (
                        "Transmettez ces informations au chercheur. "
                        "Il devra scanner ce QR code avec Google Authenticator "
                        "et confirmer via POST /api/auth/totp/confirm/."
                    ),
                },
    ...
    @staticmethod
    def suspendre(profil_id: str, effectue_par: User) -> ServiceResult:
        profil = ChercheurRepository.get_by_id(profil_id)
        if not profil:
            return ServiceResult(success=False, message="Chercheur introuvable.", http_status=404)
        if profil.activation_suspendue:
            return ServiceResult(success=False, message="Ce compte est deja suspendu.", http_status=400)

        ChercheurRepository.suspendre(profil)
        HAS.log_utilisateur('DESACTIVATION', auteur=effectue_par, cible_user=profil.user, details={'action': 'suspension'})
        return ServiceResult(success=True, message="Compte suspendu.", data={'statut_compte': profil.statut_compte}, http_status=200)
    ...
    def reactiver(profil_id: str, effectue_par: User) -> ServiceResult:
        ...
        ChercheurRepository.reactiver(profil)
        HAS.log_utilisateur('REACTIVATION', auteur=effectue_par, cible_user=profil.user, details={'action': 'reactivation'})
        return ServiceResult(success=True, message="Compte reactive.", data={'statut_compte': profil.statut_compte}, http_status=200)
