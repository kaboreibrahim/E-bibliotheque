...
# =============================================================================
# 🔷  BASE ABSTRAITE — Personnel académique (sans expiration automatique)
# =============================================================================

class PersonnelAcademiqueBase(SafeDeleteModel):
    """
    Modele de base partage entre EnseignantChercheur et Chercheur.
    Ces comptes ne gerent pas de fenetre de validite : ils restent actifs
    indefiniment et ne peuvent etre suspendus/reactivés que par un admin.
    """
    ...
    numero_piece = models.CharField(
        max_length=100,
        blank=True,
        verbose_name="Numero de piece d'identite",
    )
    ...
    annee_service = models.PositiveIntegerField(
        null=True,
        blank=True,
        verbose_name="Annee de service",
        help_text="Annee d'entree en service a l'universite.",
    )
    specialite_enseignee = models.CharField(
        max_length=255,
        blank=True,
        verbose_name="Specialite enseignee / domaine de recherche",
    )
    activation_suspendue = models.BooleanField(
        default=False,
        verbose_name="Compte suspendu",
        help_text="Indique si le compte a ete suspendu par un administrateur.",
    )
    ...
    def suspendre_compte(self):
        """Suspendre le compte (admin uniquement)."""
        if not self.activation_suspendue:
            self.activation_suspendue = True
            self.save(update_fields=["activation_suspendue", "updated_at"])
        if self.user.is_active:
            self.user.is_active = False
            self.user.save(update_fields=["is_active", "updated_at"])

    def reactiver_compte(self):
        """Reactiver le compte (admin uniquement)."""
...
class PersonneExterne(SafeDeleteModel):
    ...
    def est_dans_periode_validite(self, reference=None) -> bool:
        ...
        return self.date_debut_validite <= today <= self.date_fin_validite
    ...
    def est_expire(self) -> bool:
        if not self.compte_expire_le:
            return False
        expire_le = self.compte_expire_le
        if timezone.is_naive(expire_le):
            expire_le = timezone.make_aware(expire_le)
        return timezone.now() > expire_le
    ...
    def statut_compte(self) -> str:
        ...
        if timezone.localdate() < self.date_debut_validite:
...
class EnseignantChercheur(PersonnelAcademiqueBase):
    """
    Profil d'un enseignant-chercheur.
    Memes acces que Personne Externe, compte permanent (pas d'expiration).
    Seul l'administrateur peut creer, modifier, suspendre ou supprimer ce compte.
    """
    ...
    user = models.OneToOneField(
        ...
        limit_choices_to={"user_type": User.UserType.ENSEIGNANT_CHERCHEUR},
        verbose_name="Compte utilisateur",
    )
    ...
    def __str__(self):
        return f"Enseignant-Chercheur : {self.user.get_full_name()}"
...
class Chercheur(PersonnelAcademiqueBase):
    """
    ...
    user = models.OneToOneField(
        ...
        limit_choices_to={"user_type": User.UserType.CHERCHEUR},
        verbose_name="Compte utilisateur",
    )
    ...
    def __str__(self):
        return f"Chercheur : {self.user.get_full_name()}"
