...
from apps.users.serializers.auth_serializers import (
    ...
    EnseignantChercheurLoginSerializer, EnseignantChercheurTOTPVerifySerializer,
    ChercheurLoginSerializer, ChercheurTOTPVerifySerializer,
)
from apps.users.services.auth_service import (
    ...
    AdminAuthService,
    EnseignantChercheurAuthService,
    ChercheurAuthService,
    CommonAuthService,
)
...
class EnseignantChercheurLoginView(APIView):
    ...
    @extend_schema(
        ...
        summary='Connexion enseignant-chercheur (étape 1/2) — Email + Mot de passe',
        description="Connexion par email + mot de passe. Le TOTP Google Authenticator est requis en étape 2.",
        request=EnseignantChercheurLoginSerializer,
        responses={
            200: OpenApiResponse(description="Identifiants valides — TOTP requis ou configuration TOTP demandée."),
            401: OpenApiResponse(description="Email ou mot de passe incorrect."),
            403: OpenApiResponse(description="Compte suspendu."),
        },
    )
    def post(self, request):
        s = EnseignantChercheurLoginSerializer(data=request.data)
        if not s.is_valid():
            return Response({'success': False, 'errors': s.errors, 'data': {}, 'message': 'Données invalides.'}, status=400)
        m = _meta(request)
        return _resp(EnseignantChercheurAuthService.login(
            email=s.validated_data['email'],
            password=s.validated_data['password'],
            ip=m['ip'], ua=m['ua'],
        ))
...
class EnseignantChercheurTOTPVerifyView(APIView):
    ...
    @extend_schema(
        tags=['Auth — Enseignant-Chercheur'],
        summary='Connexion enseignant-chercheur (étape 2/2) — Code Google Authenticator',
        description="Valide le code TOTP et retourne les tokens JWT.",
        request=EnseignantChercheurTOTPVerifySerializer,
        responses={
            200: OpenApiResponse(description="Connexion réussie."),
            400: OpenApiResponse(description="Code TOTP invalide ou expiré."),
            404: OpenApiResponse(description="Enseignant-chercheur introuvable."),
        },
    )
    def post(self, request):
        s = EnseignantChercheurTOTPVerifySerializer(data=request.data)
        if not s.is_valid():
            return Response({'success': False, 'errors': s.errors, 'data': {}, 'message': 'Données invalides.'}, status=400)
        return _resp(EnseignantChercheurAuthService.verify_totp(
            user_id=str(s.validated_data['user_id']),
            totp_code=s.validated_data['totp_code'],
            ip=_meta(request)['ip'],
        ))
...
class ChercheurLoginView(APIView):
    ...
    @extend_schema(
        tags=['Auth — Chercheur'],
        summary='Connexion chercheur (étape 1/2) — Email + Mot de passe',
        description="Connexion par email + mot de passe. Le TOTP Google Authenticator est requis en étape 2.",
        request=ChercheurLoginSerializer,
        responses={
            200: OpenApiResponse(description="Identifiants valides — TOTP requis ou configuration TOTP demandée."),
            401: OpenApiResponse(description="Email ou mot de passe incorrect."),
            403: OpenApiResponse(description="Compte suspendu."),
    ...
    def post(self, request):
        s = ChercheurLoginSerializer(data=request.data)
        if not s.is_valid():
            return Response({'success': False, 'errors': s.errors, 'data': {}, 'message': 'Données invalides.'}, status=400)
        m = _meta(request)
        return _resp(ChercheurAuthService.login(
            email=s.validated_data['email'],
            password=s.validated_data['password'],
            ip=m['ip'], ua=m['ua'],
        ))
...
class ChercheurTOTPVerifyView(APIView):
    ...
    @extend_schema(
        tags=['Auth — Chercheur'],
        summary='Connexion chercheur (étape 2/2) — Code Google Authenticator',
        description="Valide le code TOTP et retourne les tokens JWT.",
        request=ChercheurTOTPVerifySerializer,
        responses={
            200: OpenApiResponse(description="Connexion réussie."),
            400: OpenApiResponse(description="Code TOTP invalide ou expiré."),
            404: OpenApiResponse(description="Chercheur introuvable."),
        },
    )
    def post(self, request):
        s = ChercheurTOTPVerifySerializer(data=request.data)
        if not s.is_valid():
            return Response({'success': False, 'errors': s.errors, 'data': {}, 'message': 'Données invalides.'}, status=400)
        return _resp(ChercheurAuthService.verify_totp(
            user_id=str(s.validated_data['user_id']),
            totp_code=s.validated_data['totp_code'],
            ip=_meta(request)['ip'],
        ))
