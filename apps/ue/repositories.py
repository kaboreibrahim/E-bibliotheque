...
class UERepository:
    ...
    def get_all() -> QuerySet:
        return UE.objects.prefetch_related("specialites", "ecues").all()

    @staticmethod
    def get_by_id(ue_id: str) -> UE | None:
        return UE.objects.prefetch_related("specialites", "ecues").filter(pk=ue_id).first()
    ...
    @staticmethod
    def get_by_specialite(specialite_id: str) -> QuerySet:
        return UE.objects.prefetch_related("specialites", "ecues").filter(
            specialites__id=specialite_id
        )
    ...
    @staticmethod
    def create(code: str, name: str, specialite_ids: list[str]) -> UE:
    ...
    def update(ue: UE, **fields) -> UE:
        specialite_ids = fields.pop("specialite_ids", None)
        for attr, value in fields.items():
            setattr(ue, attr, value)
        ue.save(update_fields=list(fields.keys()))
        if specialite_ids is not None:
            ue.specialites.set(specialite_ids)
        return ue
