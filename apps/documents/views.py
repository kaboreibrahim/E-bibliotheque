...
TYPE_DOCUMENT_RESPONSE_EXAMPLE = OpenApiExample(
    ...
    value={
        ...
        "ordre": 1,
        "niveaux": [
            {"id": "aaaa0001-0000-0000-0000-000000000000", "name": "L1", "filiere_name": "Droit"},
            {"id": "aaaa0002-0000-0000-0000-000000000000", "name": "L2", "filiere_name": "Droit"},
        ],
        "nb_documents": 8,
...
TYPE_DOCUMENT_CREATE_EXAMPLE = OpenApiExample(
    "Payload creation type de document",
    request_only=True,
    value={
        "code": "RAPPORT",
...
TYPE_DOCUMENT_UPDATE_EXAMPLE = OpenApiExample(
    "Payload mise a jour partielle (PATCH)",
    request_only=True,
    value={
        "ordre": 3,
        "niveaux_ids": ["aaaa0003-0000-0000-0000-000000000000"],
    },
)
...
DOCUMENT_LIST_PARAMETERS = [
    ...
    OpenApiParameter(
        name="search",
        description="Recherche sur titre, auteur, description, specialite ou UE.",
        required=False,
        type=str,
    ),
]
    ...
    value=[
        {
            ...
            "file_size": 204800,
            "file_stream_url": "http://testserver/api/documents/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/fichier/",
...
DOCUMENT_CREATE_REQUEST_EXAMPLE = OpenApiExample(
    "Payload creation document multipart",
    request_only=True,
    value={
...
DOCUMENT_DASHBOARD_STATS_EXAMPLE = OpenApiExample(
    "Statistiques dashboard documents",
    response_only=True,
    status_codes=["200"],
    value={
        "documents_count": 125,
        "documents_with_file_count": 125,
        "total_file_size": 73400320,
        "total_file_size_mb": 70.0,
        "total_file_size_display": "70 Mo",
        "server_disk_capacity_mb": 100,
        "server_disk_capacity_bytes": 104857600,
        "server_disk_capacity_display": "100 Mo",
        "remaining_storage_mb": 30.0,
        "remaining_storage_bytes": 31457280,
        "remaining_storage_display": "30 Mo",
        "used_storage_percentage": 70.0,
    },
)
...
@extend_schema_view(
    list=extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                name="niveau",
                description="Filtrer par UUID de niveau (retourne uniquement les types associés à ce niveau).",
                required=False,
                type=OpenApiTypes.UUID,
            ),
        ],
    ...
    create=extend_schema(
        ...
        description=(
            ...
            "Le champ `niveaux_ids` permet d'associer le type à un ou plusieurs niveaux d'études. "
            "Le champ `ordre` contrôle la position dans les listes (0 = en premier)."
        ),
        ...
        examples=[TYPE_DOCUMENT_CREATE_EXAMPLE, TYPE_DOCUMENT_RESPONSE_EXAMPLE],
    ),
    ...
    update=extend_schema(
        summary="Modifier un type de document (PUT)",
        tags=["Types de documents"],
        request=TypeDocumentSerializer,
        responses={
            200: OpenApiResponse(response=TypeDocumentSerializer, description="Type mis a jour."),
            400: OpenApiResponse(description="Donnees invalides."),
            403: OpenApiResponse(description="Permission insuffisante."),
            404: OpenApiResponse(description="Type de document introuvable."),
        },
        examples=[TYPE_DOCUMENT_UPDATE_EXAMPLE, TYPE_DOCUMENT_RESPONSE_EXAMPLE],
    ),
    partial_update=extend_schema(
        summary="Modifier partiellement un type de document (PATCH)",
        tags=["Types de documents"],
        request=TypeDocumentSerializer,
        responses={
            200: OpenApiResponse(response=TypeDocumentSerializer, description="Type mis a jour."),
    ...
    destroy=extend_schema(
        ...
        tags=["Types de documents"],
        responses={
            204: OpenApiResponse(description="Type supprime."),
            403: OpenApiResponse(description="Permission insuffisante."),
            404: OpenApiResponse(description="Type de document introuvable."),
        },
    ),
...
class TypeDocumentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not niveau_id and getattr(self.request.user, 'is_etudiant', False):
            try:
                niveau_id = str(self.request.user.profil_etudiant.niveau_id)
            except AttributeError:
                pass
        if niveau_id:
            queryset = queryset.filter(niveaux__id=niveau_id)
        return queryset
...
@extend_schema_view(
    ...
    create=extend_schema(
        ...
        description=(
            "Creation via `multipart/form-data` avec `file_path`. "
            "Le contenu est stocke comme fichier et servi en streaming."
        ),
        responses={
            201: OpenApiResponse(
...
class DocumentViewSet(viewsets.ViewSet):
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                response=DocumentDashboardStatsSerializer,
                description=(
                    "Statistiques agregées des documents visibles par l'utilisateur "
                    "courant, incluant la taille totale des fichiers."
                ),
            ),
        },
        examples=[DOCUMENT_DASHBOARD_STATS_EXAMPLE],
    )
    @action(detail=False, methods=["get"], url_path="stats/dashboard")
    def stats_dashboard(self, request):
        stats_data = _service.get_dashboard_stats(user=request.user)
        return Response(DocumentDashboardStatsSerializer(stats_data).data)
    ...
    @extend_schema(
        ...
        parameters=[DOCUMENT_ID_PARAMETER],
        description=(
            "Retourne les metadonnees du document, l'URL de lecture en flux et "
            "enregistre automatiquement une consultation de type VUE pour "
    ...
    @extend_schema(
        summary="Lire le fichier d'un document",
        tags=["Documents"],
        parameters=[
            ...
            OpenApiParameter(
                ...
                description="Mettre a `true` ou `1` pour forcer le telechargement.",
            ),
        ],
        responses={
            200: OpenApiResponse(
    ...
    def fichier(self, request, pk=None):
        ...
        if not document.file_path:
            return Response(
                {"detail": "Fichier introuvable pour ce document."},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        response = FileResponse(
            ...
            content_type=document.file_mime_type or DEFAULT_DOCUMENT_MIME_TYPE,
        )
