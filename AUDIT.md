...
| TECH-005 | ✔️ **Résolu le 2026-09-23** (corrigé une seconde fois le même jour — voir ci-dessous). `apps/documents/migrations/0012_...` utilisait du SQL MySQL (`MODIFY COLUMN`), incompatible PostgreSQL, bloquant toute base fraîche. | **Erreur n°1** : ajouter une migration corrective *après* la 0012 ne fonctionne pas (Django rejoue les migrations dans l'ordre, 0012 plante avant d'atteindre le correctif). **Erreur n°2, plus grave** : le correctif appliqué ensuite (0012 réécrite en `ALTER COLUMN ... TYPE`, syntaxe PostgreSQL) n'a été testé que sur le PostgreSQL local — **la production cPanel tourne réellement sur MySQL/MariaDB** (confirmé par le développeur : erreur 1064 MariaDB en conditions réelles lors d'un `manage.py migrate` sur le serveur, sur la nouvelle migration 0018 qui contenait la même syntaxe Postgres-only). Le déploiement prod a été bloqué par ce correctif. **Correctif définitif** : les migrations 0012 et 0018 utilisent maintenant `migrations.AlterField` (mécanisme Django standard) au lieu de SQL brut — Django génère automatiquement la syntaxe correcte pour le moteur réellement connecté (`MODIFY COLUMN` sur MySQL, `ALTER COLUMN ... TYPE` sur PostgreSQL), sans coder en dur un moteur particulier. Revérifié : `manage.py test` sur base PostgreSQL fraîche (31/31 sur `apps.documents`) ; **le développeur doit relancer `manage.py migrate` sur cPanel pour confirmer côté MySQL**, non re-testable depuis l'environnement local. Leçon retenue : le projet a deux moteurs de BDD actifs selon l'environnement (Postgres en dev local, MySQL en prod cPanel) — toute migration doit rester portable (`AlterField`/opérations Django standards), jamais de SQL brut propre à un seul moteur, sauf nécessité absolue et testée sur les deux. | 🟠 | ✔️ Terminée (à confirmer sur cPanel) |
...
| DEC-022 | 2026-09-23 | **Échec du déploiement cPanel** signalé par le développeur : `manage.py migrate` plante sur la migration 0018 (erreur MariaDB 1064) — la production tourne réellement sur MySQL, pas PostgreSQL. Le correctif TECH-005 précédent (SQL PostgreSQL brut) était incomplet, testé uniquement en local. | Migrations 0012 et 0018 réécrites avec `migrations.AlterField` (portable, génère le SQL correct pour n'importe quel moteur Django). Revérifié en local (Postgres, 31/31). **Reste à confirmer par le développeur sur cPanel** (MySQL) — action suivante : relancer `manage.py migrate` sur le serveur. |
